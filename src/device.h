#ifndef __SGI_DEVICE_H__
#define __SGI_DEVICE_H__
...
// Based on: IRIX 6.5 keyboard(7) man page on techpubs.sgi.com
// Codes are the scan codes plus one
#define LEFTCTRLKEY	CTRLKEY
#define LEFTALTKEY	84
#define RIGHTALTKEY	85
#define RIGHTCTRLKEY	86
#define F1KEY		87
#define F2KEY		88
#define F3KEY		89
#define F4KEY		90
...
typedef long Device;		// qread returns a long, so we match that -- LJE

// getqentry() returns an entry in the queue if one is present.  It
// returns positive button device values if a button goes down,
// negative values if the button went up. This was *not* one of the
// original Iris GL functions. -- LJE
extern __LINKAGE__ Device getqentry();

// Emulated Iris GL functons:
extern __LINKAGE__ void qreset(void);
extern __LINKAGE__ long qread(short *data);
extern __LINKAGE__ GLboolean getbutton(Device num);
extern __LINKAGE__ long getvaluator(Device val);
extern __LINKAGE__ void setvaluator(Device val, short init, short min,
				    short max);
extern __LINKAGE__ long qtest(void);

// GLUT event handlers:

extern __LINKAGE__ void mouse_button_event_handler(int button, int state,
						   int x, int y);
extern __LINKAGE__ void keyboard_event_handler(unsigned char key, int x, int y);
extern __LINKAGE__ void motion_event_handler(int x, int y);
extern __LINKAGE__  void drag_event_handler(int x, int y);

#endif	// __SGI_DEVICE_H__
