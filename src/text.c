...
/* GLuint font text string drawing routines begin here */


  GLfloat
compute_insert_height(textfontype thefont, GLfloat yscale)
{
...
draw_insertion_point(textfontype thefont,textstringptr textstring)
{
  ...
  glColor4f((((GLfloat) red[0]) / 255.0F),
	    ...
	    1.0F);
  glTranslatef(textstring->string_offset[vx], textstring->string_offset[vy], 	    textstring->string_offset[vz]);
  glScalef(textstring->string_x_scale, textstring->string_y_scale, 1.0);
  ...
  glLineWidth((GLfloat)(textstring->string_pen_width+1));
...
draw_obj_label(objptr labelled_obj)
{
  ...
  glMultMatrixf((GLfloat *) labelled_obj->xform);
  ...
  while (this_string_ep != Nil)
  {
    ...
    if (textstring->string != Nil)
    {
      ...
      {
      case Bold:
	glLineWidth((GLfloat)(textstring->string_pen_width*2));
	break;
      case Shadow:
	...
	break;
      case Plain:
	glLineWidth((GLfloat)(textstring->string_pen_width));
	break;
      }
      glColor4f((((GLfloat) textstring->string_color[0]) / 255.0F),
		(((GLfloat) textstring->string_color[1]) / 255.0F),
		(((GLfloat) textstring->string_color[2]) / 255.0F),
		1.0F);
...
/* GLuint font text string utility routines begin here */

  GLfloat
text_string_width(char *thestring,textfontype thefont,GLfloat string_x_scale)
{
  int i = 0;
  GLfloat width = 0;
  ;
...
create_text_string(char *new_string,vertype string_offset,
		   ...
		   GLfloat string_x_scale,GLfloat string_y_scale,
		   GLfloat string_pen_width,colortype string_color,
...
set_label_item_offset(objptr labelled_obj, int item_number,vertype new_offset)
{
  ...
  void
set_label_item_scale(objptr labelled_obj, int item_number,GLfloat new_x_scale,
		     GLfloat new_y_scale)
{
  textstringptr text_string;
  ;
...
objstr(char *thestring,vertype pos,GLfloat x_scale, GLfloat y_scale)
{
  int i = 0;
  ;
  glPushMatrix();
